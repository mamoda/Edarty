...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-emerald-100 selection:text-emerald-900" dir="rtl">
      {/* Header */}
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-lg shadow-sm py-3' : 'bg-transparent py-5'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            {/* Logo */}
            <div className="flex items-center gap-2 cursor-pointer group" onClick={() => scrollToSection('hero')}>
              <div className="relative">
                <img src={logo} alt="Edarty Logo" className="h-12 w-auto transition-transform group-hover:scale-105" />
              </div>
              <span className={`text-2xl font-bold tracking-tight transition-colors ${scrolled ? 'text-slate-900' : 'text-slate-900'}`}>إدارتــي</span>
            </div>
            ...
            <nav className="hidden md:flex items-center gap-8">
              {['features', 'benefits', 'pricing', 'testimonials'].map((item) => (
                ...
                >
                  {item === 'features' ? 'المميزات' : item === 'benefits' ? 'الفوائد' : item === 'pricing' ? 'الأسعار' : 'آراء العملاء'}
                </button>
              ))}
            </nav>
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white border-t border-slate-100 shadow-xl py-6 px-6 animate-in fade-in slide-in-from-top-5">
            <nav className="flex flex-col gap-4">
              <button onClick={() => scrollToSection('features')} className="text-right py-2 text-slate-600 font-medium">المميزات</button>
              <button onClick={() => scrollToSection('benefits')} className="text-right py-2 text-slate-600 font-medium">الفوائد</button>
              <button onClick={() => scrollToSection('pricing')} className="text-right py-2 text-slate-600 font-medium">الأسعار</button>
              <div className="h-px bg-slate-100 my-2"></div>
              <button onClick={handleLogin} className="text-right py-2 text-slate-900 font-bold">تسجيل الدخول</button>
              <button onClick={handleFreeTrial} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-bold">ابدأ تجربة مجانية</button>
            </nav>
          </div>
        )}
      </header>
      ...
      <section id="features" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-20">
            <h2 className="text-emerald-600 font-bold text-sm uppercase tracking-widest mb-3">المميزات التقنية</h2>
            <p className="text-3xl lg:text-4xl font-black text-slate-900">كل ما تحتاجه في مكان واحد</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: <BarChart3 />, title: "تقارير ذكية", desc: "تحليلات دقيقة لحظة بلحظة لمبيعاتك ومصروفاتك." },
              { icon: <Shield />, title: "أمان فائق", desc: "تشفير بياناتك بأعلى المعايير العالمية لضمان الخصوصية." },
              { icon: <Users />, title: "إدارة الفريق", desc: "توزيع الصلاحيات ومتابعة أداء الموظفين بكل سهولة." },
              { icon: <DollarSign />, title: "الفواتير الآلية", desc: "إصدار فواتير احترافية متوافقة مع المتطلبات الضريبية." },
              { icon: <Clock />, title: "توفير الوقت", desc: "أتمتة العمليات المتكررة لتركز على نمو عملك." },
              { icon: <Lock />, title: "نسخ احتياطي", desc: "حفظ تلقائي لبياناتك لضمان عدم فقدان أي معلومة." }
            ].map((feature, idx) => (
              <div key={idx} className="p-8 rounded-3xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:shadow-2xl hover:shadow-slate-200/50 transition-all group">
          ...
          <div className="grid md:grid-cols-4 gap-12 mb-16">
            <div className="col-span-2">
              ...
              <p className="text-slate-400 text-lg max-w-md leading-relaxed">
                نحن هنا لنغير مفهوم الإدارة المحاسبية. انضم إلى آلاف الشركات التي تثق في إدارتي لتطوير أعمالها.
              </p>
            </div>
            <div>
